...
const onPropertySelect = (value: string) => {
  const property = PROPERTIES.find(prop => prop.id === value) || null
...
async function generatePDF(event: Event) {
  ...
  try {
    ...
    const elementsToHide = [
      ...
      '.pdf-buttons',
      '.q-table__bottom',
      '.damage-record-list'  // Añadir lista de daños
    ]
    ...
    const damageMarkers = clonedForm.querySelectorAll('.damage-marker')
    ...
    // Capturar imagen del formulario con opciones mejoradas
    const canvas = await html2canvas(clonedForm, {
      ...
      onclone: (documentClone) => {
        // Asegurar que las marcas de daño estén fijas en el clon
        const clonedDamageMarkers = documentClone.querySelectorAll('.damage-marker')
        clonedDamageMarkers.forEach((marker) => {
          const markerElement = marker as HTMLElement
          markerElement.style.position = 'absolute'
          markerElement.style.zIndex = '10'
        })
    ...
    const pageWidth = pdf.internal.pageSize.getWidth() - 20  // Restar márgenes
    const pageHeight = pdf.internal.pageSize.getHeight() - 20  // Restar márgenes
