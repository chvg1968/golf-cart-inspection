...
async function generatePDF() {
  try {
    ...
    // Generar PDF solo si la referencia existe
    if (pdfGeneratorRef.value) {
      pdfGeneratorRef.value.generatePDF(pdfData)
    } else {
      throw new Error('Referencia al generador de PDF no disponible')
    }
  } catch (error) {
...
function onPDFError(error: Error) {
  quasar.notify({
    type: 'negative',
    message: 'Error al generar PDF. Consulte la consola.',
    position: 'top'
  })
