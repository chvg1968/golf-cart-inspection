...
export default {
  ...
  export const CART_PARTS = [
      ...
      export default {
        props: {
          cartParts: {
            type: Array,
            default: () => CART_PARTS // Valor por defecto
          }
        }
      }
  ...
  setup(props, { emit }) {
    // Function to get the label of a part from its value
    const getPartLabel = (partValue) => {
      // Search in predefined parts
      
      
      // First search in parts provided by props
      const part = props.cartParts.find(p => p.value === partValue) || 
                  defaultParts.find(p => p.value === partValue)
      
      return part ? part.label : partValue
    }
    
    // Function to get the label of a damage type from its value
    const getDamageTypeLabel = (typeValue) => {
      ...
      // First search in types provided by props
      const type = props.damageTypes.find(t => t.value === typeValue) || 
                  defaultTypes.find(t => t.value === typeValue)
