<template>
  ...
  async function generatePDF(data?: PDFData) {
    try {
      ...
      // Capturar canvas con estrategia de reintento
      const captureCanvasWithRetry = async (maxAttempts = 3): Promise<HTMLCanvasElement> => {
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
          try {
            ...
            // Validar dimensiones
            console.log(`Intento ${attempt} - Dimensiones del canvas:`, canvas.width, canvas.height)
            ...
            console.error(`Error en intento ${attempt} de captura:`, error)
      ...
      // Capturar canvas
      const canvas = await captureCanvasWithRetry()
