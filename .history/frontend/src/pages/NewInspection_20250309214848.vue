...
const generatePDF = async () => {
  ...
  try {
    ...
    // Mantener solo la tabla de daños
    const damageRecordListContainer = clonedContent.querySelector('.damage-record-list')
    const damageRecordListClone = damageRecordListContainer ? damageRecordListContainer.cloneNode(true) : null
    ...
    // Si hay tabla de daños, añadirla como una página adicional
    if (damageRecordListClone) {
      // Crear div temporal para la tabla de daños
      const damageTableDiv = document.createElement('div')
      damageTableDiv.appendChild(damageRecordListClone)
      document.body.appendChild(damageTableDiv)

      // Capturar tabla de daños
      const damageCanvas = await html2canvas(damageTableDiv, { 
        scale: 1.5,
        logging: false,  
        useCORS: true,
        allowTaint: true,
      ...
      document.body.removeChild(damageTableDiv)

      // Convertir imagen de tabla de daños
      const damageImgData = damageCanvas.toDataURL('image/jpeg', 0.7)

      // Añadir página de tabla de daños
      pdf.addPage()
      
      // Calcular dimensiones para la tabla de daños
      const damageImgWidth = pageWidth - (2 * margin)
      const damageImgHeight = (damageCanvas.height * damageImgWidth) / damageCanvas.width
    ...
    console.log('Number of pages:', totalPages + (damageRecordListClone ? 1 : 0))
