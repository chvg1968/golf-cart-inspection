...
const isFormValid = computed(() => {
  ...
  // Validación de marca en el diagrama
  const hasDiagramMark = damages.value.some(damage => 
    damage.position && damage.position.x !== undefined && damage.position.y !== undefined
  )

  // Validación de términos y firma
  const termsAndSignatureValid = termsAccepted.value && signature.value

  return guestInfoValid && 
         termsAndSignatureValid
})
