<template>
  ...
  async function generatePDF(data?: PDFData) {
    try {
      ...
      // Crear un div temporal con el formulario clonado
      ...
      clonedForm.style.width = '100%'
      clonedForm.style.height = 'auto'
      clonedForm.style.opacity = '1'
      clonedForm.style.visibility = 'visible'
      ...
      originalMarkers.forEach((originalMarker, index) => {
        ...
        clonedMarker.style.opacity = '1'
        clonedMarker.style.visibility = 'visible'
      })

      // Añadir formulario clonado al div temporal
      tempDiv.appendChild(clonedForm)
      document.body.appendChild(tempDiv)

      // Esperar un breve momento para renderizado
      ...
      const canvas = await html2canvas(clonedForm, {
        ...
        scrollX: 0,
        scrollY: 0,
        // Configuración para evitar document.write
      ...
      if (canvas.width === 0 || canvas.height === 0) {
        throw new Error('Canvas generado sin contenido')
      }
      ...
      // Convertir canvas a imagen base64 de alta calidad
