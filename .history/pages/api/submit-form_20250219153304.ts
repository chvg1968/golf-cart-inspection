...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Crear un único registro en Airtable con toda la información
    const airtableRecord = {
      ...
      "Preview observations by Guest": observations,
      "Signature Checked": signatureChecked,
      "Front Left Scratches": frontLeftSide.scratches,
      "Front Right Scratches": frontRightSide.scratches,
      "Front Left Missing Parts": frontLeftSide.missingParts,
      "Front Right Missing Parts": frontRightSide.missingParts,
      "Front Left Damage/Bumps": frontLeftSide.damageBumps,
      "Front Right Damage/Bumps": frontRightSide.damageBumps
    };

    // Enviar registro a Airtable
    const airtableResponse = await fetch(
      `https://api.airtable.com/v0/${process.env.AIRTABLE_BASE_ID}/${process.env.AIRTABLE_TABLE_NAME}`,
      {
        ...
        body: JSON.stringify({ fields: airtableRecord }),
      }
    );

    const airtableResult = await airtableResponse.json();

    // Verificar si el registro se envió correctamente
    if (airtableResult.error) {
      logWithDiagnostics('❌ ERROR: No se pudo guardar el registro en Airtable', airtableResult, 'error');
      throw new Error('Error al guardar en Airtable: ' + airtableResult.error.message);
    }
    ...
    // Preparar correo electrónico con PDF adjunto
    const emailData = {
      ...
      attachments: [{
        filename: `golf-cart-inspection-${inspectionId}.pdf`,
        content: pdfBuffer
      }]
    };
    ...
    const responseData = { 
      ...
      airtableResults: {
        success: !airtableResult.error,
        details: airtableResult.error ? 'Error al guardar el registro' : 'Registro guardado exitosamente'
      },
