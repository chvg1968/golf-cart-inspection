...
async function generatePDF(event: Event) {
  ...
  try {
    ...
    // Asegurar que las marcas de daño estén correctamente posicionadas
    ...
    damageMarkers.forEach((marker) => {
      ...
      // Obtener coordenadas originales del marcador
      const originalMarker = document.querySelector(`[data-marker-id="${markerElement.dataset.markerId}"]`)
      if (originalMarker) {
        const rect = originalMarker.getBoundingClientRect()
